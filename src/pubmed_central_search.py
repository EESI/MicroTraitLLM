...
def question_formation(prompt,model,temperature,api_key,ncbi_api_key):
    ...
    if model == "ChatGPT-4o-mini":
        ...
        if re_search_term is not None:
            ...
            link = f'https://eutils.ncbi.nlm.nih.gov/entrez/eutils/esearch.fcgi?db=pmc&retmode=json&retmax=20&sort=relevance&term={search_term}+AND+open+access[filter]&api_key=83e5ed87f0d4e27474b0681f748747f6e908'
        else:
            search_term = None
            link = None
